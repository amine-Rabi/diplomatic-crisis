...
const GAME = "diplomatic_crisis" as const;
...
export async function getGameResult(gameId: string): Promise<JudgingResult | null> {
  ...
  try {
    ...
    console.warn(
      "[crisis getGameResult] readContract failed (is the deployed contract up to date with get_game_result?):",
      e,
    );
    return null;
  }
  ...
  try {
    ...
    console.warn("[crisis getGameResult] parse failed:", e);
    return null;
  }
}
...
function isValidResult(obj: any): obj is JudgingResult {
  return obj && typeof obj === "object" &&
    ...
    typeof obj.results[0]?.wit === "number" &&
